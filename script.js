...
sabmit.addEventListener('click', () => {
  ...
  $nilaiHarga.innerHTML = rupiah.format($hargaBarang.value);
  $uang2.innerHTML = rupiah.format($uang.value);
  const $hasil = $jumlahBarang.value * $hargaBarang.value;
  $total.innerHTML = rupiah.format($hasil);
  $kembalian.innerHTML = rupiah.format($uang.value - $hasil);
...
tambah.addEventListener("click", () => {
  ...
  kasir.addEventListener('submit', function(e) {
    ...
    for (let i = 0; i < namaBarang.length; i++) {
      ...
      hargaNilai.innerHTML = rupiah.format(nilaihrg);
      uang2.innerHTML = rupiah.format(uang.value);
      const aray = [];
      ...
      total.innerHTML = rupiah.format(hasilnya);
      kembalian.innerHTML = rupiah.format(uang.value - hasilnya);
